...
&emsp; &emsp; 方法2：使用哈希表，存储数字与下标的映射值。然后遍历数组，看是否存在 target - left 的值<br/>
...
&emsp; &emsp; 动态规划，每次记录下最左侧的下标，然后遍历字符串的同时更新新的左侧，并更新当前字母出现的位置，以及更新当前所能获得的最大长度即可。<br/>
...
&emsp; &emsp; 方法 1：变种二分查找，先找到等于目标值的下标，然后在该下标左右再分别进行二分查找，找到 nums[n] == target && nums[n - 1] != target 以及 nums[m] == target && nums[m + 1] != target 的位置。<br/>
&emsp; &emsp; 时间复杂度 O(logn)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：使用二分查找分别找到左边的界限以及右边的界限，当找到为当前值时不返回而是根据所要找的界限不同继续循环，直到不满足循环条件。<br/>
&emsp; &emsp; 时间复杂度 O(logn)，空间复杂度 O(1)。
...
&emsp; &emsp; 由于题目保证了一定能跳到最后的格子，所以能跳到最远位置的格子一定是最优的格子。因此减少了需要作为回溯的遍历的次数，时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 3：进行双指针 dp，使用数组 dp 记录下动态规划所用的数组，用一个指针指向最前面能达到当前位置的节点 pre，另一个指针指向当前遍历到的节点 cur。cur 从 1 开始遍历，找到第一个能到达当前位置的 pre，由于题目保证存在结果，因此 pre 总能被找到，然后 dp[cur] = dp[pre] + 1。返回 dp[numsSize - 1] 即为结果。<br/>
...
&emsp; &emsp; 方法 1：经典贪心题。相加至当前值小于零时，将当前总和置零即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：使用分治法，声明一个类记录左右分段的当前子序和 iSum，当前最大的子序和 mSum，当前的左半段的最大子序和 lSum，右半段的最大子序和 rSum。<br/>
&emsp; &emsp; 合并条件：<br/>
&emsp; &emsp; iSum = l.iSum + r.iSum<br/>
&emsp; &emsp; lSum = max(l.lSum, l.iSum + r.lSum)<br/>
&emsp; &emsp; rSum = max(r.rSum, r.isum + l.rSum)<br/>
&emsp; &emsp; mSum = max(l.mSum, r.msum, l.rSum + r.lSum)<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(logn)。较复杂，然而当把分段和记录成树，则可以在 O(logn) 的时间内得到任意区间的最大分段和。
...
&emsp; &emsp; 方法 1：经典贪心题目，参照 45 题做法。只需遍历 size - 2 项，当 i 等于上次的最远位置且最远位置没有被更新时，返回 false，否则返回 true。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：动态规划，使用变量记录当前最后的位置 last，当 last <= i + nums[i] 时，将 last 更新为 i。最后判断 i 是否等于 0。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度为 O(logn)。<br/>
...
&emsp; &emsp; 动态规划，使用两个指针，分别表示当前能获得的最大乘积 maxMultiple 以及最小乘积 minMultiple，然后通过这两个乘积获得下一个乘积以排除符号。然后通过一个数字记录已获得的最大乘积 result，每次对比 result 与 maxMultiple 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度为 O(1)。
...
&emsp; &emsp; 使用带 GROUP BY 的子查询，子查询中查找最小的 id，然后排除掉最小的 id 即可。<br/>
...
&emsp; &emsp; 进行递归，获得从某一位开始分割后，左边和右边数字的可能性，然后将这些可能性进行相应的数学运算得到结果。<br/>
&emsp; &emsp; 时间复杂度 O($n^2$)，空间复杂度 O($n^2$)。

## 242. 有效的字母异位词

&emsp; &emsp; 送分题，使用 map 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 257. 二叉树的所有路径

&emsp; &emsp; 送分题，进行 DFS 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(H)。

## 258. 各位相加

&emsp; &emsp; 根据观察可知，一个数如果是 9 的倍数，则其各位相加将会是 9 的倍数。而 9 恰好是比 10 小的数，因此可以直接将当前数字对 9 取余得到结果。为避免 9 的倍数对 9 取余后为 0，因此需要将原数字 -1 后对 9 取余再加 1。<br/>
&emsp; &emsp; 时间复杂度 O(1)，空间复杂度 O(1)。

## 260. 只出现一次的数字 III

&emsp; &emsp; 官方做法：先对所有数字进行异或，得到 xOrSum = result1 ^ result2。<br/>
&emsp; &emsp; 然后通过 lastBitIsOne = xOrSum & (-xOrSum) 得到 XOrSum 中最低位的 1。由于 result1 != result2，所以 xOrSum 必不等于 0，所以 xOrSum 必然存在一个最低位的 1，而 result1 和 result2 必然在最低位的 1 上不相等。<br/>
&emsp; &emsp; 再次遍历数组，若 lastBitIsOne & num == 0，则 result1 ^= num，否则 result2 ^= num。而由于剩余的数字都出现了两次，再次遍历数组进行异或运算时，必然会被重置，所以第二次遍历后能得到最终结果。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 262. 行程和用户

&emsp; &emsp; 查询出一个子表，选择出 request_at 在目标日其中且 client_id 以及 driver_id 不在 ban 掉的 id 中的 trip。然后查询该子表，使用 COUNT(IF()) 来统计未完成的旅程数，使用 ROUND 来保留小数点后 n 位同时对小数点后 n 位进行四舍五入即可。
...
&emsp; &emsp; 方法 1：使用最小堆，每次取最小堆的堆顶元素，分别乘以 2、3、5 然后放入堆中。<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(n)。<br/>
&emsp; &emsp; 方法 2：使用动态规划，使用记录下上次乘以 2、3、5 时各自的指针位置，然后通过这些指针的位置分别乘以 2、3、5 将最小的值赋给 dp[i]。然后查看 dp[i] 是否等于这些积，若等于某一个的积，则当前指针向前移。最后返回 dp[n] 即为结果。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 268. 丢失的数字

&emsp; &emsp; 方法 1：当 nums[i] != i 且 nums[i] < numsSize 时，交换 nums[i] 与 nums[nums[i]] 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：由于数字数限定在 10 ^ 4 以下，通过求和公式可知总和应为 n * (n + 1) / 2，通过该总和逐个相减即可得到目标值。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
...
&emsp; &emsp; 线段树。。<br/>
&emsp; &emsp; 构造时间复杂度 O(n)，更新时间复杂度 O(logn)，计算区间和时间复杂度 O(logn)，空间复杂度 O(n)。
...
&emsp; &emsp; 从 1 遍历到 amount，并从 0 遍历到 coinsSize - 1，dp 条件为 dp[i] = min(dp[i], dp[i - coins[j]] + 1)，看拿了该硬币后是否硬币个数多于之前。最后判断 coins[amount] 是否大于 amount，是则表示没有满足条件的选法，返回 -1。否则返回 coins[amount]。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(n)。<br/>
...
&emsp; &emsp; 方法 1：对 t 求和后再减去 s。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：同时对两个字符串进行异或。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 392. 判断子序列

&emsp; &emsp; 方法 1：使用双指针即可。<br/>
&emsp; &emsp; 时间复杂度 O(m + n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：使用动态规划。使用类似于 KMP 的思想，建立一个长为 26 宽为 m + 1 的数组 dic，记录下 t 中每一个字母下一次出现的下标，若不存在则以 n 代替。第 m 行为 guard 行，保证不存在的情况不会越过 dp 数组。<br/>
...
&emsp; &emsp; 从 0 开始遍历 data，记录 data 的前置 1 的长度。<br/>
&emsp; &emsp; 若前置 1 等于 0，则进行下一轮循环。<br/>
&emsp; &emsp; 若前置 1 大于等于 2 且小于等于 4，则从 1 开始遍历到 count，判断这些数字的前置 1 数量是否为 1。不为 1 则返回 false，循环结束后将当前循环指向最后的位置。<br/>
&emsp; &emsp; 不满足上述条件则返回 false。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 404. 左叶子之和

&emsp; &emsp; 迭代，遍历树，判断左节点不为空的同时左右节点是否为空，是则加入结果中。<br/>
&emsp; &emsp; 时间复杂度 O(m)，空间复杂度 O(n)。
...
&emsp; &emsp; 背包问题。<br/>
&emsp; &emsp; 根据分析可知，该问题等同于求是否存在一个子数组，其总和等于原数组总和的一半。根据这个方向去找是否存在这样的背包即可。<br/>
&emsp; &emsp; 时间复杂度 O($n^2$)，空间复杂度 O(n)。

## 420. 强密码检验器

&emsp; &emsp; 模拟题，分各种情况讨论即可。<br/>
&emsp; &emsp; 时间复杂度 O((logn) ^ 2)，空间复杂度 O(1)。

## 429. N 叉树的层序遍历

&emsp; &emsp; 送分题，BFS。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 440. 字典序的第K小数字

&emsp; &emsp; 根据模拟可知，节点 i 的子结点树为 10 * i - 10 * i + 9，通过先序遍历找到第 k - i 的节点，然后在从后向前找，直到计算出到达 k 的序号即可。<br/>
&emsp; &emsp; 时间复杂度 O((logn) ^ 2)，空间复杂度 O(1)。

## 496. 下一个更大元素 I

&emsp; &emsp; 单调栈，从后向前遍历数组，若当前数字大于等于栈顶，弹出。然后判断栈是否为空，为空则当前数字的下一个更大元素为 -1，否则为栈顶元素。<br/>
&emsp; &emsp; 时间复杂度 O(m)，空间复杂度 O(m)。

## 504. 七进制树

&emsp; &emsp; 送分题，使用绝对值对 7 取余的数字放入结果，然后将绝对值 /= 7，直到绝对值等于 0 时退出循环即可。若 num < 0 则需要在最后放入负号，然后反转原结果字符串得到最终结果。同时需要注意对 num 等于 0 时蓄特殊处理，直接返回 0 即可。<br/>
&emsp; &emsp; 时间复杂度 O(logn)，空间复杂度 O(1)。

## 509. 斐波那契数

&emsp; &emsp; 方法 1：使用长度为 2 的滚动数组进行 dp 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：使用矩阵的思想，根据分析可知，可写成 {{1, 1}, {1, 0}} 的对 {f(n), f(n-1)} 的矩阵乘积，然后得到的 result[0][0] 即为最后结果。同时可使用快速幂的方式节省时间。<br/>
&emsp; &emsp; 时间复杂度 O(logn)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 3：根据矩阵幂得到矩阵的特征根进行计算。<br/>
&emsp; &emsp; 时间复杂度与空间复杂度取决于 pow 的实现。

## 518. 零钱兑换 II

&emsp; &emsp; 322 题变种。采用与 322 题一样的 dp 方式的话会导致重复情况的发生。因此外层使用硬币，内层从硬币值 dp 到 amount 以避免重复的选择硬币的方式。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(n)。

## 521. 最长特殊序列 Ⅰ

&emsp; &emsp; 智力题，比较 a 与 b 是否相等，若相等返回 1，否则返回最长的长度。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 542. 01 矩阵

&emsp; &emsp; BFS，先使用堆或队列记录当前为 0 的下标作为起始条件，遍历堆，然后将其四周为 1 的元素赋值为当前的距离 -1，加入到下一轮的堆中。最后，将所有数组的元素取反即可。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(1)。

## 564. 寻找最近的回文数

&emsp; &emsp; 先编写函数用以算出当前字符串能得到的回文串，构造方式为将前半部分复制到后半部分。<br/>
&emsp; &emsp; 然后从个位一直遍历到最高位，依次减 10 ^ n 直到得到第一个比当前数字小的回文数。然后对原数字依次加 10 ^ n 直到得到第一个比当前数字大的回文数。通过对比这两个回文数与原数字的差值大小以得到最终的结果。<br/>
&emsp; &emsp; 同时，对于原数字为 11 的情形需要特殊处理，返回 9 以避免得到不正确的结果。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(1)。
...
&emsp; &emsp; 方法 1：DFS暴力解法。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(m)。

## 584. 寻找用户推荐人

&emsp; &emsp; 送分题，用 IS NULL 判断为 NULL 的情况即可。
...
&emsp; &emsp; DFS，若当前节点为空，返回空，然后判断左节点和右节点，若左节点右节点都为空则直接返回，若左节点为空则加入空白括号即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(H)。

## 607. 销售员

&emsp; &emsp; 先连接 Company 和 Orders 为表 t，条件为公司为 red，然后使 SalesPerson 与 t 做左链接，得到 t 表中 sales_id IS NULL 的数据即可。

## 608. 树节点

&emsp; &emsp; 使用两重 IF 语句即可。

## 627. 变更性别

&emsp; &emsp; UPDATE TABLE SET sex = IF() 即可。<br/>
&emsp; &emsp; 或者使用
...
&emsp; &emsp; 广度优先搜索加上 map 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 661. 图片平滑器

&emsp; &emsp; 二维前缀和即可。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(m * n)。

## 682. 棒球比赛

&emsp; &emsp; 使用栈保存数值即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。
...
&emsp; &emsp; 200 题岛屿问题变种。DFS 即可。<br/>
&emsp; &emsp; 时间复杂度 O(mn)，空间复杂度 O(mn)。
...
&emsp; &emsp; 对字符串排序后，使用哈希表记录各个字符串然后判断每个字符串前缀是否出现过即可。<br/>
&emsp; &emsp; 时间复杂度 O($n^2$)，空间复杂度 O(n)。

## 728. 自除数

&emsp; &emsp; 暴力法。<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(1)。

## 733. 图像渲染

&emsp; &emsp; DFS。当下标超出界限或当前元素与目标值一致的时候返回。确定好初始值，当当前元素与初始值一致时，替换为目标值，进行下一层检索。当当前元素与目标值一致时，直接返回即可。<br/>
&emsp; &emsp; 时间复杂度 O(mn)，空间复杂度 O(mn)。

## 740. 删除并获得点数

&emsp; &emsp; 198 题变种。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(m)，m 为数组的最大值。
...
&emsp; &emsp; 暴力，统计每一个数字的二进制 1 的个数，判断。<br/>
&emsp; &emsp; 同时，由于 32 以内的质数都已知，可以使用一个数字 mask 记录为这些质数的位或值，然后通过对 1 进行左移当前数字的二进制 1 的个数，与 mask 进行位与判断是否为 0 从而判断是否为质数。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 780. 到达终点

&emsp; &emsp; 反向查找，通过 tx 与 ty 互相取余，然后判断是否 tx 等于 sx 且 ty 等于 sy，若为真，则返回 true。否则判断 tx 等于 sx 时，且 ty 对 sx 取余是否等于 sy。以及 ty 等于 sy 时，tx 对 sy 取余是否等于 sx。<br/>
&emsp; &emsp; 时间复杂度 O(logn)，空间复杂度 O(1)。

## 796. 旋转字符串

&emsp; &emsp; 根据分析可知，如果 s 为 goal 的旋转字符串，那么 s + s 必然存在 goal 的子串，从而变成找子串是否存在的问题。使用 kmp 来找子串即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(m)。
...
&emsp; &emsp; 记忆化搜索。自顶向下迭代，每次得到 [a - 100, b - 0],[a - 75, b - 25],[a - 50, b - 50],[a - 25, b - 75] 时 A 分完的概率。然后通过 Dictionary 记录下该概率避免重复计算即可。<br/>
&emsp; &emsp; 根据题意要求误差值在 $10^{-5}$ 内，而当 n 大于一定值时，概率将为 1，根据计算可取 n >= 4500 时直接返回 1。<br/>
&emsp; &emsp; 时间复杂度 O($n^2$)，空间复杂度 O(n)。
...
&emsp; &emsp; 120 的变种，同样思路进行 dp 即可。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(m)。

## 953. 验证外星语词典

&emsp; &emsp; 使用长为 256 的数组记录下新的字典顺序，然后判断相邻的两个字符串是否满足字典序即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 961. 在长度 2N 的数组中找出重复 N 次的元素

&emsp; &emsp; 方法 1：使用一个长度为 3 的数组记录下出现过的数字，每次更新其中的数字。如果存在其中相同的数字即为结果。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：使用随机数，直到随机到最终结果。<br/>
&emsp; &emsp; 时间复杂度 O(1)，空间复杂度 O(1)。<br/>

## 965. 单值二叉树

&emsp; &emsp; DFS，送分题。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 976. 三角形的最大周长

&emsp; &emsp; 对数组排序后，从后向前遍历，找到第一个 nums[i - 1] + nums[i - 2] > nums[i] 的值，作为三角形的三条边记录周长即可。<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(1)。
...
&emsp; &emsp; 根据分析，如果下一个节点比当前节点更大，则取该节点，否则，只有当下一个节点找到比该节点更大的节点时，才能进一步判断是否满足当前节点，即使用单调栈的思路完成即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 1020. 飞地的数量

&emsp; &emsp; DFS，DFS 的条件为当前格子等于 1，则将当前格子置为 -1，并向四周扩散。整个函数执行完后，将 -1 置为 1 避免更改原数组。<br/>
&emsp; &emsp; 按照题意，与边界接触的块不满足条件，因此先从边界开始 DFS，去除所有与边界接触的块。然后再遍历内部的项即可。<br/>
&emsp; &emsp; 每个元素最多被访问 5 次，因此时间复杂度 O(m * n)，空间复杂度 O(1)。

## 1023. 驼峰式匹配

&emsp; &emsp; 双指针，每次判断下标的元素是否相同，或者 query 中的元素是否为大写字母即可。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(1)。

## 1026. 节点与其祖先之间的最大差值

&emsp; &emsp; 直接进行 DFS 即可，并在 DFS 参数中传递最大最小值即可。<br/>
&emsp; &emsp; 而且在遍历到叶子节点时，由于最大最小值通过参数传递下去，因此可只在叶子节点时更新最大最小值节省时间。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 1039. 多边形三角剖分的最低得分

&emsp; &emsp; 区间 DP。每次都能先确定一条边，然后选择另一个点构成三角形时，将拆分成一个三角形和两个多边形，同时这两个多边形又能各自确定一条边，进行同样的步骤。<br/>
&emsp; &emsp; 然后每层 DP 都是确定当前的选中的边所能得到的最低得分即可。<br/>
&emsp; &emsp; 时间复杂度 O(n ^ 3)，空间复杂度为 DFS 时的空间 O(n ^ 2)。

## 1041. 困于环中的机器人

&emsp; &emsp; 根据分析，如果处于环中，那么机器人的最终方向肯定与初始方向不同，或者经过一轮循环后处于原点。<br/>
&emsp; &emsp; 因此时间复杂度 O(n)，空间复杂度 O(1)。

## 1042. 不邻接植花

&emsp; &emsp; 先使用 vector<vector<int>>(n) 用于记录接临边的信息 adj，然后遍历该接临边信息，记录各个边的花的种植情况，然后将未种过的花作为当前花园的花即可。由于题目中已限定每个花园最多有 3 条路径，因此最多的花数为 4 种，可以在统计未种过的花时进行简化。<br/>
&emsp; &emsp; m 为花园个数，n 为路径个数。因为每个花园最多遍历 3 次，因此时间复杂度为 O(m + n)，空间复杂度 O(m + n)。

## 1137. 第 N 个泰波那契数

&emsp; &emsp; 509 题的变种，根据 509 题斐波那契数列的思想即可。<br/>
&emsp; &emsp; 使用 dp 的时间复杂度 O(n)，空间复杂度 O(1)。或使用快速幂的时间复杂度 O(logn)，空间复杂度(1)。

## 1144. 递减元素使数组呈锯齿状

&emsp; &emsp; 贪心，每次取当前下标与两边的差值的最大值，统计到最终结果中即可，然后每次下标数值加 2。因此需要分从 0 开始以及从 1 开始的情形。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 根据数学公式，直线方程为 ax + by + c = 0，将全部坐标减去 coordinates[0] 的 X 和 Y，从而得到一条过原点的直线，从而有 ax + by = 0。以 coordinates[1] 的坐标为 a、-b，从 2 开始遍历判断是否满足 ax - by = 0 即可。<br/>
&emsp; &emsp; 空间复杂度 O(logn)，空间复杂度 O(1)。

## 1247. 交换字符使得字符串相同

&emsp; &emsp; 统计同样下标下 s1[i] 为 x，s2[i] 为 y，计为 xy；以及 s1[i] 为 y 而 s2[i] 为 x 的情况，计为 yx。通过分析可以得知通过一次交换可以使 xy 或者 yx 的计数 -2。<br/>
&emsp; &emsp; 而当 xy 或 yx 为奇数时，通过一次交换可以使 xy + 1 而 yx - 1 或者反之。<br/>
&emsp; &emsp; 综合以上可以得到公式 result = xy / 2 + yx / 2 + xy % 2 + yx % 2。<br/>
&emsp; &emsp; 空间复杂度 O(logn)，空间复杂度 O(1)。

## 1281. 整数的各位积和之差

&emsp; &emsp; 根据分析，从外往内缩一圈，进行 DFS。<br/>
&emsp; &emsp; DFS 返回 bool 类型，代表当前 DFS 是否能组成陆地。可知当在边界且不为陆地时，当前 DFS 不能组成陆地。<br/>
&emsp; &emsp; 同时需注意，不能直接使用短路，避免部分情形不完整的 DFS 且当前格子的值被更改时在其他 DFS 被判定为陆地。<br/>
&emsp; &emsp; 时间复杂度 O(mn)，空间复杂度 O(mn)
...
&emsp; &emsp; 送分题。<br/>
&emsp; &emsp; 空间复杂度 O(n)，空间复杂度 O(1)。

## 1309. 解码字母到整数映射

&emsp; &emsp; 遍历字符串同时判断 i + 2 是否为 # 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 方法 1：使用 map 统计每个位数出现的数字，然后从 1 遍历到 14，对 map 内的数组排序再添加到结果中即可。<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(n)。<br/>
&emsp; &emsp; 方法 2：直接自定义 sort 函数即可。<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(1)。

## 1484. 按日期分组销售产品

&emsp; &emsp; 考察 GROUP_CONCAT 和 COUNT(DISTINCT) 的使用。

## 1491. 去掉最低工资和最高工资后的工资平均值

&emsp; &emsp; 一边统计当前的总和，一边统计当前最大最小值即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 找到规律，得到最终个数为 (high - low) / 2 + ((high % 2) || (low % 2))<br/>
&emsp; &emsp; 时间复杂度 O(1)，空间复杂度 O(1)。

## 1527. 患某种疾病的患者

&emsp; &emsp; 方法 1：使用 LIKE 语句进行条件筛选。<br/>
&emsp; &emsp; 方法 2：使用 LEFT 判断是否以 'DIAB1' 开始，再使用 LOCATE 函数判断是否存在 ' DIAB1'。
...
&emsp; &emsp; 根据题意，需要删除一个子数组，该删除数组可能以右侧为边界，左侧为边界，或者边界都在中间，分情况讨论。<br/>
&emsp; &emsp; 先考虑左侧为边界的情况，则从右往左遍历，直至左侧的值大于右侧，可得到右侧有序数组的左边界 right 以及得到应删除的左侧子数组的长度。<br/>
&emsp; &emsp; 然后根据分析，由于此时已知右侧有序数组的左边界，left 从 0 开始遍历，如果 nums[right] >= nums[left]，则 right - left 即为删除的中间子数组的长度，否则， right 应当向右移得到中间子数组的长度。<br/>
&emsp; &emsp; 而当删除的数组为右侧数组时，中间数组总会由于 right++ 扩展到右侧数组，即可以同时考虑到该情况。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 1581. 进店却未进行过交易的顾客

&emsp; &emsp; 左连接后，GROUP BY 即可。

## 1582. 二进制矩阵中的特殊位置

&emsp; &emsp; 根据分析，统计每一行 1 出现的次数，以及每一列 1 出现的次数，然后当当前行、列的 1 出现个数为 1 且数组中当前元素为 1 即满足题意。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(m + n)。

## 1588. 所有奇数长度子数组的和

&emsp; &emsp; 方法 1：暴力，每次遍历然后统计奇数子数组和。<br/>
&emsp; &emsp; 时间复杂度 O(n ^ 3)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：使用前缀和数组记录下各个位置的前缀和以减少重复计算。<br/>
&emsp; &emsp; 时间复杂度 O($n^2$)，空间复杂度 O(n)。<br/>
&emsp; &emsp; 方法 3：根据数学分析，得到每一位数字在子数组中出现的次数。<br/>
&emsp; &emsp; 由分析可知，当一个数字在奇数的子数组中出现时，左右的数字个数应同时为奇数或偶数。因此统计出左边奇数情况的个数乘以右边奇数的个数，偶数同理，两者相加即可得到当前数字出现的次数。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 根据题意，rowSum[i]，colSum[i] >= 0，可知使用贪心，总能使每个格子得到值。贪心的条件即为 min(rowSum[i], colSum[j])，因为不满足条件的格子总能用 0 补齐。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(1)。

## 1630. 等差子数组
&emsp; &emsp; 1. 获取每个子数组的最大值与最小值，得到每个子数组如果为等差数列时的差值。<br/>
&emsp; &emsp; 2. 如果最大值等于最小值，则当前子数组满足条件，然后判断 (maxValue - minValue) 是否能被长度 (right - left) 整除，不能则不满足条件。能则得到 interval。<br/>
&emsp; &emsp; 3. 建立长度为 right - left + 1 的 bool 数组作为哈希表，判断子数组中每个数字是否能够整除 interval，不能则当前子数组不满足条件，能则判断 (num - minValue) / interval 即在哈希表中的下标是否出现过，出现过则当前子数组不满足条件。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(1)。

## 1637. 两点之间不包含任何点的最宽垂直区域

&emsp; &emsp; 方法 1：排序后遍历即可。<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：桶排序，先找到最大值 maxValue 与最小值 minValue，max((maxValue - minValue) / (n - 1), 1) 获取到每个桶的上下限之差，然后 (maxValue - minValue) / bucketSize + 1 获取到桶的个数。<br/>
&emsp; &emsp; 初始化桶为 vector<pair<int, int>> buckets(bucketCount, pair<int, int>{INT32_MAX, INT32_MIN})，pair.first 为当前桶的下限值，pair.second 为当前桶的上限值。<br/>
&emsp; &emsp; 然后，遍历每个 point，(point[0] - minValue) / bucketSize 即为当前桶所在的下标。并更新当前桶的上下限值。<br/>
&emsp; &emsp; 最后，遍历每个桶，当桶的下限值大于上限值时，该桶未被使用，即非法。然后得到每个桶上下限的差值，以及下一个有效的桶下限值与上一个有效的桶的上限值的差值。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 1638. 统计只差一个字符的子串数目

&emsp; &emsp; 方法 1：暴力，s 从 0 开始，t 从 0 开始，依次遍历每个子串下标的字符是否相同，若子串不同的字符数为 1，则结果加 1，若大于 1，则退出当前子串的循环。<br/>
&emsp; &emsp; 时间复杂度 O(m * n * min(m, n))，空间复杂度 O(1)。
&emsp; &emsp; 方法 2：中心扩展法，方法 1 的优化版，找当前不同的字符，将两边的不同字符向两端进行扩展，然后统计左端与右端相同字符的数目，两者的数目分别 +1 再相乘即为当前中心所能得到的不同子串个数。<br/>
&emsp; &emsp; 时间复杂度 O(m * n * min(m, n))，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 3：动态规划，分别记录每个下标对应的最长公共前缀长度以及公共后缀长度，最后遍历两个字符串，当当前字符不一致时，使用同上的方法即可得到当前下标的不同字串个数。通过 dp 数组减少了统计时的遍历次数。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(m * n)。<br/>
&emsp; &emsp; 方法 4：动态规划优化，参考链接 <a href="https://leetcode.cn/problems/count-substrings-that-differ-by-one-character/solutions/2192600/tu-jie-fei-bao-li-onm-suan-fa-pythonjava-k5og/">【图解】非暴力 O(nm) 算法（Python/Java/C++/Go）</a>。<br/>
&emsp; &emsp; 从 1 - sLength 遍历到 tLength，外层循环意义为 s 中的起始下标与 t 中的起始下标之差。<br/>
&emsp; &emsp; 第二层循环中，将两个起始坐标分别右移作为结束坐标，然后通过记录上一个不同的下标以及上上次不同的下标，该两个下标之差即当前结束下标能得到的符合题意的字符串个数。<br/>
&emsp; &emsp; 时间复杂度 O(m * n)，空间复杂度 O(m * n)。

## 1641. 统计字典序元音字符串的数目

&emsp; &emsp; 方法 1：动态规划，类似于斐波那契数列。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：排列组合。将问题可视为将 n 个球放入 5 个篮子中，且篮子的球数目可为空。最后公式即为 $C\binom{4}{n + 4}$<br/>
&emsp; &emsp; 时间复杂度 O(1)，空间复杂度 O(1)。
...
&emsp; &emsp; 考察 SQL 函数 CONCAT，UPPER，LOWER，LEFT，RIGHT 的使用。

## 1678. 设计 Goal 解析器

&emsp; &emsp; 由于题目保证了输入，所以可以不使用 substr 直接使用 i + 1 进行判断。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 使用数组记录下不同的下标，当数组长度大于2时返回 false。然后判断数组长度是否为 0，为 0 则返回 true，为 2 则判断是否交换过后即可满足条件。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 1791. 找出星型图的中心节点

&emsp; &emsp; 根据题意，由于数字各不相同，所以只需要判断两组边即可得到结果。<br/>
&emsp; &emsp; 时间复杂度 O(1)，空间复杂度 O(1)。

## 1792. 最大平均通过率

&emsp; &emsp; 使用最大堆，记录各个班级增加一个通过学生时增加的通过率。<br/>
&emsp; &emsp; 遍历 n 次，每次从最大堆取出班级，并将处理后的班级在最后插入最大堆中。最后遍历最大堆统计最后的平均通过率即可。<br/>
&emsp; &emsp; 时间复杂度 O((n + m)logm)，空间复杂度 O(m)。

## 1795. 每个产品在不同商店的价格

&emsp; &emsp; 列转行，且没有排序以及去重的要求，使用 UNION ALL 而不是 UNION 提高效率。

## 1805. 字符串中不同整数的数目

&emsp; &emsp; 遍历，注意去除前导 0，使用 HashSet 记录下已有的数字。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 使用 IF(contition, true command, false command) 进行逻辑判断后选择相应的值。<br/>
&emsp; &emsp; 使用 MOD 获取取模得到奇数，使用 LEFT 获取名字的首字母。

## 1965. 丢失信息的雇员

&emsp; &emsp; 使用 UNION ALL 然后 GROUP BY HAVING COUNT(*) = 1 即可。
...
&emsp; &emsp; 滑动窗口，声明一个函数用于判断不包含某个字符的最大字符串长度，然后判断窗口长度即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 统计连续出现的字母个数，大于 3 则可以开始计数。最后返回 Alice 和 Bob 的计数谁更多即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。
...
&emsp; &emsp; 使用预处理加前缀和。分别统计每一个位置最左边的蜡烛位置以及每一个位置最右边的蜡烛位置，如果不存在则以 -1 作为特殊标志位。<br/>
&emsp; &emsp; 然后对 query[0] 取右边的蜡烛的位置，对 query[1] 取左边蜡烛的位置，保证在该区间存在蜡烛的重叠区域即可。<br/>
&emsp; &emsp; 时间复杂度 O(n + q)，q 为查询的次数，空间复杂度 O(n)。

## 2100. 适合打劫银行的日子

&emsp; &emsp; 方法 1：DP，使用标志位分别记录当前天数的左侧满足条件的天数以及右边满足条件的天数，若满足则 +1，否则置为 0。然后判断这天是否前 time 天和后 time 天都满足条件，若满足条件则将该天数加入考虑中。<br/>
&emsp; &emsp; 时间复杂度时为 O(n)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：DP，使用数组记录左右满足条件的天数，然后从 1 遍历到 n，依次判断左右天数是否满足条件。<br/>
&emsp; &emsp; 时间复杂度时为 O(n)，空间复杂度 O(n)。<br/>

## 2104. 子数组范围和

&emsp; &emsp; 方法 1：两次遍历。<br/>
&emsp; &emsp; 时间复杂度 O($n^2$)，空间复杂度 O(1)。<br/>
&emsp; &emsp; 方法 2：使用单调栈。<br/>
&emsp; &emsp; 思想为以某个数为界限，找到该左边以及右边最近的比他小的数字的下标，最近的比他大的数字下标，然后在各个子序列中，然后约定当左边与右边相等时也视为左边的数比右边的数小。<br/>
&emsp; &emsp; 得到 leftMin、leftMax、rightMin、rightMax 后，(rightMax[i] - i) * (i - leftMax[i]) 即为该数作为最大值使用的次数。(rightMin[i] - i) * (i - leftMin[i]) 即为该数作为最小值使用的次数。统计 maxSum 以及 minSum，然后 maxSum - minSum 即可得到最终的结果。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。<br/>

## 2169. 得到 0 的操作数

&emsp; &emsp; 循环，直到 num1 或 num2 等于 0 时跳出。循环条件为先得到最小数字，然后 num1 = num1 + num2 - minValue * 2; num2 = minValue 即可。<br/>
&emsp; &emsp; 时间复杂度当其中一者为 1 时为 O(n)，空间复杂度 O(1)。

## 2170. 使数组变成交替数组的最少操作数

&emsp; &emsp; 统计奇偶各个数字出现的次数，第一次遍历哈希表，统计并判断两个哈希表中的最大值的数字是否相同。<br/>
&emsp; &emsp; 若相同，则从哈希表中去除相同的数字，再次遍历得到两个哈希表中的次大值，根据两个最大值与次大值的交错和的大小获得最少操作数。<br/>
&emsp; &emsp; 若不同，直接返回数组长度减去两个最大值即可得到最少操作数。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 2363. 合并相似的物品

&emsp; &emsp; 使用 unordered_map<int, int> 统计每个商品的重量和。然后将每个商品的信息放到 vector<vector<int>> 中。对该 vector 调用 sort 即可。<br/>
&emsp; &emsp; 也可使用内置了红黑树排序的 map 一步到位。<br/>
&emsp; &emsp; 时间复杂度 O(nlogn)，空间复杂度 O(n)。

## 2367. 算术三元组的数目

&emsp; &emsp; 方法 1：使用 unordered_set，然后遍历数组，判断是否存在 num - diff 以及 num - 2 * diff 即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。<br/>
&emsp; &emsp; 方法 2：使用三指针，分别为左、第一个 diff 的元素、以及第二个 diff 的元素的下标。找到则 reuslt + 1。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。<br/>
...
&emsp; &emsp; 由于需要精力和经验，先对 energy 进行求和并 +1 得到总共需要的精力，然后对经验进行模拟，其中可以对经验的公式进行简化。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 2395. 和相等的子数组

&emsp; &emsp; 使用 unordered_set<int> 记录子数组的和即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 2399. 检查相同字母间的距离

&emsp; &emsp; 使用数组记录每个字母上次出现的位置然后判断即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 2404. 出现最频繁的偶数元素

&emsp; &emsp; 使用 unordered_map<int, int> 记录各个数字出现的个数然后对偶数元素进行判断即可。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。

## 2427. 公因子的数目

&emsp; &emsp; 通过 gcd 函数获取到两束的最大公约数，然后从 0 判断到 sqrt(divisor) 即可，判断 divisor 能否整除 i。同时，如果 divisor 是平方数，结果再加 1。<br/>
&emsp; &emsp; 时间复杂度 O($\sqrt{n}$)，空间复杂度 O(1)。
...
&emsp; &emsp; 根据分析，即统计包含 K 的子数组中，小于 K 的数字个数等于大于 K 的数字个数（奇数中位数），或者等于大于 K 的数字个数 -1 的情况（偶数中位数）。<br/>
&emsp; &emsp; 为简化统计，将小于 K 的数字计为 -1，将大于 K 的数字计为 1，则问题可转换为包含 K 的子数组中前缀和为 N 或 N - 1 的个数。<br/>
&emsp; &emsp; 首先得到 K 所在的下标，然后从 0 到 indexOfK 进行遍历，计算前缀和，此时的下标可作为子数组的左边界，记录现在得到的前缀和到字典中。从 indexOfK 遍历到 n，计算前缀和，此时的下标可作为子数组的右边界。而以该下标为右边界时，前缀和为 N 或 N - 1 的数目为当前能得到的中位数为 K 的子数组数目，对这些数目求和，即可得到最终结果。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(n)。
...
&emsp; &emsp; 根据数学分析，当前 [xPos, yPos] 所在的正方形螺旋的起始序号等于外层正方形螺旋的总元素个数 % 9 + 1，因此求出外层正方形螺旋的总元素个数即可得到问题的解。<br/>
&emsp; &emsp; 外层正方形螺旋的总元素个数为当前二维矩阵的元素个数减去内层正方形矩阵的元素个数。需要根据 xPos 和 yPos 计算出当前位置所在的正方形矩阵的宽。<br/>
&emsp; &emsp; 时间复杂度 O(1)，空间复杂度 O(1)。

## 剑指 Offer 05. 替换空格

&emsp; &emsp; 先统计空格的个数，然后将原字符串调用 resize 方法扩充到应有的长度，接着从后向前遍历字符串依次赋值即可。<br/>
&emsp; &emsp; 通过直接 resize 避免了字符串被销毁的开销。<br/>
&emsp; &emsp; 时间复杂度 O(n)，空间复杂度 O(1)。

## 剑指 Offer 06. 从尾到头打印链表

&emsp; &emsp; 送分题，用数组记录后反转数组即可。

## 剑指 Offer 09. 用两个栈实现队列

&emsp; &emsp; LeetCode 232 同题。
...
&emsp; &emsp; LeetCode 155 同题。

## 剑指 Offer 35. 复杂链表的复制

&emsp; &emsp; LeetCode 138 同题。

## 剑指 Offer 53 - I. 在排序数组中查找数字 I

&emsp; &emsp; LeetCode 34 题同题。<br/>
&emsp; &emsp; 时间复杂度 O(logn)，空间复杂度 O(1)。

## 剑指 Offer 53 - II. 0～n-1中缺失的数字

&emsp; &emsp; 二分查找找到第一个下标不为 n 的下标。<br/>
&emsp; &emsp; 时间复杂度 O(logn)，空间F复杂度 O(1)。

## 程序员面试金典 面试题 05.02. 二进制数转字符串

&emsp; &emsp; 根据题意，判断当前数字与 1 的对比值，如果当前值等于 1 则在最后一位加 1 然后返回即可。大于 1 则在最后一位加 1，否则在最后一位加 0。每次结束后，将当前值乘以 2 进行下一次循环即可，且循环次数需限制在 32 位内。<br/>
&emsp; &emsp; 由于循环次数上限为 32，时间复杂度 O(1)，空间复杂度 O(1)。
